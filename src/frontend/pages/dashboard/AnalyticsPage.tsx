...
export default function AnalyticsPage() {
  ...
  const topLinksWithPct = topLinks.map((l) => ({
    ...l,
    pct: totalClicks > 0 ? (l.clicks / totalClicks) * 100 : 0,
  }))
  ...
  const clicksPath = daily
    .map((d, i) => `${i === 0 ? 'M' : 'L'} ${xScale(i)} ${yScale(d.clicks)}`)
    .join(' ')
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Clicks, views & link performance — last 7 days
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Calendar className="h-4 w-4" /> {daily[0]?.date ?? '—'} —{' '}
      ...
      {!hasData && (
        <div className="card p-8 text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Share your profile <span className="font-mono">/@{user?.username}</span> and
            clicks/views will appear here.
          </p>
        </div>
      )}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card p-4 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {topLinks.length} link{topLinks.length !== 1 ? 's' : ''} clicked
          </p>
        </div>
        <div className="card p-4 space-y-2">
          <div className="flex items-center justify-between">
        ...
        <div className="card p-4 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="h-8 w-8 rounded-lg bg-green-50 flex items-center justify-center text-green-600">
        ...
        <div className="card p-4 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="h-8 w-8 rounded-lg bg-amber-50 flex items-center justify-center text-amber-600">
          ...
          {mostClicked ? (
            <>
              <p
                ...
                title={mostClicked.title || mostClicked.url || ''}
              >
                {mostClicked.title || mostClicked.url || '—'}
              </p>
              <p className="text-xs text-muted-foreground">
                {mostClicked.clicks} clicks •{' '}
                {((mostClicked.clicks / Math.max(1, totalClicks)) * 100).toFixed(1)}% of all
              </p>
            </>
          ) : (
        ...
        <div className="card p-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-bold flex items-center gap-1">
              <TrendingUp className="h-4 w-4 text-green-600" /> {ctr}%
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="card space-y-3 rounded-2xl p-4 lg:col-span-3">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold flex items-center gap-2">
              <Trophy className="h-4 w-4 text-amber-600" /> Ranking — Most Clicked
            </h3>
          ...
          {topLinksWithPct.length ? (
              ...
              {topLinksWithPct.map((l, i) => (
                <div key={l.linkId || i} className="space-y-1.5">
                  <div className="flex items-center gap-3">
                    <span
                      className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${i === 0 ? 'bg-amber-400 text-white' : i === 1 ? 'bg-zinc-300 text-zinc-700' : i === 2 ? 'bg-amber-700 text-white' : 'bg-muted text-muted-foreground'}`}
                    >
                      {i + 1}
                    </span>
            ...
            <p className="text-sm text-muted-foreground py-6 text-center">
              No link clicks yet — data will appear after visitors click your links.
            </p>
          )}
        </div>
        <div className="card space-y-4 rounded-2xl p-4 lg:col-span-2">
          <h3 className="font-semibold flex items-center gap-2">
            <PieChartIcon className="h-4 w-4 text-primary" /> Click Share
          </h3>
          {topLinksWithPct.length ? (
            <>
      ...
      <div className="card space-y-3 rounded-2xl p-4">
        <h3 className="font-semibold flex items-center gap-2">
          <BarChart3 className="h-4 w-4 text-primary" /> Clicks per Link
        </h3>
        {topLinks.length ? (
          <div className="space-y-2">
            {topLinksWithPct.map((l, i) => (
              <div key={l.linkId || i} className="flex items-center gap-3">
                <span
                  className="w-28 sm:w-40 text-xs font-medium truncate text-right shrink-0"
                  title={l.title || ''}
                >
                  {l.title || l.url || '—'}
                </span>
                <div className="flex-1 h-7 rounded-md bg-muted overflow-hidden relative">
                  <div
                    className="h-full rounded-md flex items-center justify-end pr-2 text-xs font-bold text-white transition-all"
                    style={{
                      width: `${Math.max(8, (l.clicks / maxClicks) * 100)}%`,
                      background: donutColors[i % donutColors.length],
                    }}
                  >
                    {l.clicks}
      ...
      <div className="card space-y-3 rounded-2xl p-4">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold flex items-center gap-2">
            <Calendar className="h-4 w-4 text-primary" /> Clicks & Views — Last 7 Days
          </h3>
        ...
        <div className="w-full overflow-x-auto">
          <svg
            ...
            className="w-full h-[180px] min-w-[520px]"
