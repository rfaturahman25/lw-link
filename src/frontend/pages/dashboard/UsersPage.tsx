...
export default function UsersPage() {
  ...
  const loadUsers = async () => {
    try {
      ...
      )) as { success: boolean; data: AdminUser[] }
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (createForm.role === 'admin' || createForm.role === 'super_admin') {
      if (!confirm(`Create ${createForm.role.toUpperCase()}? This grants elevated permissions.`))
        return
    }
    setCreating(true)
    try {
  ...
  const handleRoleChange = async (u: AdminUser, newRole: string) => {
    ...
    if (
      !confirm(
        `Change ${u.username} from ${u.role.toUpperCase()} → ${newRole.toUpperCase()}? This changes privileges.`
      )
    )
      return
    try {
  ...
  const handleDelete = async (u: AdminUser) => {
    ...
    if (
      !confirm(
        `Delete @${u.username} (${u.role})? This deletes profile & links and cannot be undone.`
      )
    )
      return
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 flex items-center gap-2 text-3xl font-bold tracking-tight">
      ...
      <div className="card space-y-3 rounded-2xl p-5">
        ...
        <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input
            ...
            placeholder="email@lensawaktu.id"
            className="input"
            type="email"
            required
          />
          ...
          <select
            ...
            onChange={(e) => setCreateForm({ ...createForm, role: e.target.value as never })}
            className="input"
          >
          ...
          <button
            type="submit"
            disabled={creating}
            className="inline-flex items-center justify-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 shadow md:col-span-2"
          >
            {creating ? (
              'Creating...'
            ) : (
              <>
                <Plus className="h-4 w-4" /> Create {createForm.role.toUpperCase()}
              </>
            )}
          </button>
        </form>
      ...
      <div className="card space-y-3 rounded-2xl p-5">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Filter className="h-4 w-4" /> Filters: {roleFilter} / {statusFilter}{' '}
          {search && `• "${search}"`}
        </div>
      </div>
            ...
            <tbody>
              {adminUsers.map((u) => (
                <tr key={u.id} className="border-b last:border-0 hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell text-xs truncate max-w-[180px]">
                    {u.email}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium ${u.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                    >
                      {u.status === 'active' ? (
